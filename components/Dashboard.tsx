...
export default function Dashboard() {
  ...
  return (
    <section
      ref={registerLabRef}
      id="lab"
      className="flex min-h-screen w-full flex-col items-center justify-center px-4 py-20 scroll-snap-start md:py-32"
    >
      <div className="w-full max-w-4xl">
        <div className="mb-12 text-center">
          <h2 className="text-2xl font-bold tracking-tight md:text-3xl">IoT Dashboard</h2>
          <p className="mt-2 text-[hsl(var(--text-muted))]">为未来单片机数据预留的可视化区域</p>
        </div>
        <div className="grid gap-6 md:grid-cols-2">
          <DashboardCard title="室内温度" delay={0} />
          <DashboardCard title="设备状态" delay={0.1} />
        </div>
      </div>
    </section>
  );
...
function DashboardCard({ title, delay }: { title: string; delay: number }) {
  return (
    <motion.div
    ...
    >
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <p className="mt-2 text-xs text-[hsl(var(--text-muted))]/70">Waiting for device connection...</p>
      </div>
    </motion.div>
  );
}
